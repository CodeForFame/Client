...
import org.moparscape.client.gui.OneonOne;
import org.moparscape.client.gui.QuestionMenuScreen;
import org.moparscape.client.gui.SmithingScreen;
import org.moparscape.client.interfaces.InterfaceGUI;
import org.moparscape.client.interfaces.InterfaceHandler;
import org.moparscape.client.model.Display;
import org.moparscape.client.model.Resolutions;
import org.moparscape.client.model.Sprite;
import org.moparscape.client.util.Config;
import org.moparscape.client.util.DataConversions;
...
public class mudclient extends GameWindowMiddleMan {
	...
	private final void loadConfigFilter() {
		drawDownloadProgress("Checking local data files", 1, false);
		EntityHandler.load();
	}

	private final void loadModels() {
		drawDownloadProgress("Loading 3d models", 75, false);
	...
	private final void drawPlayerInfoMenu(boolean flag) {
		...
		if (anInt826 == 1) {
			...
			gameGraphics.drawString("Username:@yel@ " + ourPlayer.name, i + 5,
					i1, 1, 0xffffff);
			i1 += 13;
			gameGraphics.drawString("Coords:@yel@ (" + (sectionX + areaX)
					+ ", " + (sectionY + areaY) + ")", i + 5, i1, 1, 0xffffff);
			i1 += 13;
...
//		}
	...
	private final void resetLoginVars() {
		...
		playerCount = 0;
		npcCount = 0;
		for (int i = 0; i < newQuestNames.length; i++)
			questStage[i] = 0;
		questPoints = 0;
	}
	...
	/*
	 * private int lastMouseButton = -1; private int lastMouseX = -1; private
	 * int lastMouseY = -1; private double camVert = 0.0D; private double camRot
	 * = 0.0D; private java.awt.Robot robot; protected final void
	 * handleMouseDrag(MouseEvent mouse, int x, int y, int button) {
	 * if(controlDown && button == 2) { int unitX = x - lastMouseX;
	 * 
	 * if(unitX > 1) unitX = 1; else if(unitX < -1) unitX = -1;
	 * 
	 * int unitY = lastMouseY - y;
	 * 
	 * if(unitY > 1) unitY = 1; else if(unitY < -1) unitY = -1;
	 * 
	 * cameraVertical += unitY; //cameraRotation += unitX;
	 * 
	 * System.out.println("Y: " + y + ", LastY: " + lastMouseY + " dif: (" + (y
	 * - lastMouseY) + ")");
	 * 
	 * lastMouseX = x; lastMouseY = y; //super.mouseX = lastMouseX;
	 * //super.mouseY = lastMouseY; //robot.mouseMove(super.mouseX,
	 * super.mouseY); } }
	 */
	...
	private final void checkMouseStatus() {
		/*
		 * lastMouseX =
		 * (int)java.awt.MouseInfo.getPointerInfo().getLocation().getX();
		 * lastMouseY =
		 * (int)java.awt.MouseInfo.getPointerInfo().getLocation().getY();
		 * lastMouseButton = mouseButtonClick;
		 * 
		 * if(mouseButtonClick == 2 && controlDown) return;
		 */
	...
	private final void processGame() {
		...
		if (sleeping) {
			...
			} // PLZ CAN I HAS NEW SLEEP EKWAZION?
