...
public class DataFileDecrypter {
	...
	public static byte[] load(String entry) throws IOException {
		int i = 0;
		int j = 0;
		int k = 0;
		byte abyte0[] = null;
		while (i < 2) {
			try {
				if (i == 1)
					entry = entry.toUpperCase();
				InputStream inputstream = streamFromPath(entry);
				DataInputStream datainputstream = new DataInputStream(inputstream);
				byte data[] = new byte[6];
				datainputstream.readFully(data, 0, 6);
				j = ((data[0] & 0xff) << 16) + ((data[1] & 0xff) << 8) + (data[2] & 0xff);
				k = ((data[3] & 0xff) << 16) + ((data[4] & 0xff) << 8) + (data[5] & 0xff);
				int l = 0;
				abyte0 = new byte[k];
				int i1;
				for (; l < k; l += i1) {
		...
		if (k != j) {
			byte data_[] = new byte[j];
			unpackData(data_, j, abyte0, k, 0);
			return data_;
		} else {
			return abyte0;
		}
	...
	public static int short_get(byte[] arc, int i) {
		return arc[0] * 256 + arc[1];
	}
