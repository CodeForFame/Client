...
public class Config {
	...
	public static Properties loadConfig() {
		...
		try {
			defaultProps.load(new FileInputStream(CONF_DIR + File.separator
					+ "config.properties"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Stores information into the properties + writes it to the configuration
	 * file
	 * 
	 * @param key
	 * @param value
	...
	public static void storeConfig(String key, String value) {
		...
		try {
			FileOutputStream out = new FileOutputStream(CONF_DIR
					+ File.separator + "config.properties");
			config.store(out, "MoparClassic configuration file");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
